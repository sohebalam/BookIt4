...
const UpdateRoom = () => {
  ...
  const { loading, error, isUpdated } = useSelector((state) => state.updateRoom)

  const roomDetails = useSelector((state) => state.roomDetails)
  const { loading: detailsLoading, error: detailsError, room } = roomDetails
  ...
  }, [dispatch, error, isUpdated, detailsError, room, id])
  ...
  const submitHandler = (e) => {
    ...
    dispatch(roomUpdate(room._id, roomData))
  }
