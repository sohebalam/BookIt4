...
import { NEW_ROOM_RESET } from "../../redux/constants/roomTypes"
...
const NewRoom = () => {
  ...
  const newRoom = useSelector((state) => state.newRoom)
  const { loading, error, success } = newRoom
  ...
  const submitHandler = (e) => {
    ...
    if (images.length === 0) return toast.error("Plaese upload images")

    dispatch(roomNew(roomData))
  }
